...
export class UpdateService {
    ...
    async updatePCard(payload: UpdatePCard) {
        const bigPic1 = uuidv4(), bigPic2 = uuidv4(), smlPic = uuidv4(), cardUuid = uuidv4();
        if (
        ...
        newCard.cardIdolEvents = this.UpdateUtilities.getIdolEvents(payload.idol.produceIdolEvents, payload.idol.produceAfterEvents);
        newCard.cardMemoryAppeals = this.UpdateUtilities.getMemoryAppeals(payload.idol.memoryAppeals, payload.idolId);
    ...
    async updateSCard(payload: UpdateSCard) {
        const bigPic1 = uuidv4(), smlPic = uuidv4(), cardUuid = uuidv4();
        if (
            await this.checkDuplicateUuid(bigPic1) ||
            await this.checkDuplicateUuid(smlPic) ||
            await this.checkDuplicateUuid(cardUuid)
        ...
        newCard.cardName = this.UpdateUtilities.removeSpace(payload.supportIdol.name);
        newCard.cardUuid = cardUuid;
        newCard.bigPic1 = bigPic1;
        ...
        newCard.releaseDate = fns.format(new Date(), "yyyy-MM-dd");
        newCard.cardPanels = this.UpdateUtilities.getPanels(payload.supportIdol.skillPanels);
        newCard.cardProficiencies = this.UpdateUtilities.getProficiencies(payload.musicSupportProficiencyBonuses, payload.supportIdolId);
        newCard.cardSupportEvents = this.UpdateUtilities.getSupportEvents(payload.supportIdol.produceSupportIdolEvents);
        newCard.cardSupportSkills = this.UpdateUtilities.getSupportSkills(payload.supportIdol.supportSkills);
        await this.dataSource.getRepository(CardList).save(newCard);
        await this.UpdateUtilities.getSImage(payload.supportIdolId, payload.supportIdol.hash, bigPic1, smlPic);
    }
    ...
    saveJsonFile(payload: JSON, fileName: string) {
        ...
        fs.writeFileSync(path.join(process.env.FILE_CARDJSON_PATH, fileName), JSON.stringify(payload));
    }

    async checkCardExistence(enzaId: string): Promise<void> {
        if (isEmpty(enzaId) ||
            await this.dataSource.getRepository(CardList).createQueryBuilder('cardlist').where('cardlist.enzaId = :enzaId', { enzaId: enzaId }).getOne() != null
        ) {
