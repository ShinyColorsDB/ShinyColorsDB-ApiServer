...
export class UpdateUtilities {
    idolCardPath: string = "/assets/images/content/idols/card/";
    ...
    getProficiencies(p: { proficiency: string, value: number }[], supportIdolId: string): CardProficiency[] {
        let pArray: {
            enzaId: string;
            proficiency: string;
            value: number;
    ...
    getPanels(panels: any[]): CardPanel[] {
        ...
        panels.forEach((panel: {
            ...
            if (panel.skill?.linkSkill) {
                panel.skill.linkSkill.linkSkillCharacters.forEach((cId: { character: { id: string; }; }) => {
            ...
            pArray.push({
                ...
                panelReleaseByEvent: panel.releaseConditions.match(/イベント/) ? true : false,
                panelReleaseDesc: panel.releaseConditions ? panel.releaseConditions : null,
                ...
                linkSkillDesc: panel.skill?.linkSkill ? panel.skill.linkSkill.comment : null,
                linkWith: panel.skill?.linkSkill ? linkWith : null,
                linkEffects: panel.skill?.linkSkill ? panel.skill.linkSkill.skillEffects : null,
                plusSkillId: panel.skill?.plusSkill ? panel.skill.plusSkill.id : null,
                plusSkillDesc: panel.skill?.plusSkill ? panel.skill.plusSkill.comment : null,
                plusSkillEffects: panel.skill?.plusSkill ? panel.skill.plusSkill.skillEffects : null,
            });
        });
    ...
    getMemoryAppeals(memoryAppeals: any[], idolId: string): CardMemoryAppeal[] {
        ...
        memoryAppeals.forEach((element: {
            ...
            if (element?.linkSkill) {
                element.linkSkill.linkSkillCharacters.forEach((character: { character: { id: string; }; }) => {
            ...
            mArray.push({
                ...
                memoryLinkSkillId: element?.linkSkill ? element.linkSkill.id : String(0),
                linkSkillDesc: element?.linkSkill ? element.linkSkill.comment : null,
                linkWith: element?.linkSkill ? linkWith : null,
                linkEffects: element?.linkSkill ? element.linkSkill.skillEffects : null,
            });
        });
    ...
    getCardType(enzaId: string) {
        ...
        switch (enzaId[2]) {
            case '1': rarity = 'N'; break;
            case '2': rarity = 'R'; break;
            case '3': rarity = 'SR'; break;
            case '4': rarity = 'SSR'; break;
        }
        return `${prefix}_${rarity}`;
    }

    getIdolEvents(idolEvents: any[], afterEvents: any[]): CardIdolEvent[] {
        let eventArray: { enzaId: string; eventCategory: string; eventId: number; eventHash: string; eventTitle: string }[] = new Array();
        ...
        afterEvents.forEach((event: {
            ...
            eventArray.push({ enzaId: event.idolId, eventCategory: event.eventCategoryName, eventId: Number(event.id), eventHash: null, eventTitle: event.title });
        });
        return eventArray as CardIdolEvent[];
    }

    async getPImage(enzaId: string, hash: string, bigPic1: string, bigPic2: string, smlPic: string) {
        ...
        const hash1 = this.getHash(pic1Path), hash2 = this.getHash(pic2Path), hash3 = this.getHash(smlPath);
    ...
    async getSImage(enzaId: string, hash: string, bigPic1: string, smlPic: string) {
        const pic1Path = `${this.supportIdolCardPath}${hash}_${enzaId}.jpg`,
            smlPath = `${this.supportIdolIconPath}${hash}_${enzaId}.png`;

        const hash1 = this.getHash(pic1Path), hash3 = this.getHash(smlPath);
    ...
    getHash(path: string) {
        ...
        return crypto.createHash("sha256").update(b[0] + b.at(-1) + path).digest("hex");
    }
